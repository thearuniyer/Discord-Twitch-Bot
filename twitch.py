...
# run the get_app_access_token to get new access_token because itt exprires every 60 days
...
# get stream info about said users
...
# check if the streams are online and add to list
    ...
    for user_name in config["watchlist"]:
        ...
        if user_name not in streams:
            ...
            started_at = datetime.strptime(streams[user_name]["started_at"],
                                           "%Y-%m-%dT%H:%M:%SZ")
            if online_users[user_name] is None or started_at > online_users[
                    user_name]:
